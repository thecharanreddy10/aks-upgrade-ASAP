"""Tests for pod remediation: strategy planning, approval gates, owner reference detection."""
...
def test_find_owner_reference_skips_non_workload_owners():
    pod = {
        "metadata": {
            "name": "job-pod",
            "ownerReferences": [
                {"kind": "Job", "name": "backup", "apiVersion": "batch/v1"}
            ],
        }
    }
    owner = _find_owner_reference(pod)
...
def test_find_owner_reference_none_when_no_owners():
    pod = {"metadata": {"name": "standalone-pod"}}
    owner = _find_owner_reference(pod)
    assert owner is None
...
def test_plan_delete_pod_generates_delete_command():
    pod = {"metadata": {"name": "stuck-pod"}}
    plan = _plan_delete_pod("phonebook", pod)
